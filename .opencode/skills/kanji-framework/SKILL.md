---
name: kanji-framework
description: Use when building, modifying, or debugging a backend application with Kanji Framework (Hono + DI + Zod contracts + Drizzle ORM + OAuth/JWT). Covers creating modules, controllers, services, contracts, auth guards, database schemas, migrations, OpenAPI generation, and real-time (WebSockets). Trigger on: new endpoint, new resource, new module, add auth, add validation, database query, migration, contract definition, scaffold with CLI, fix Kanji error.
---

# Kanji Framework — How to Build With It

> Kanji is a Bun-native backend framework. This skill teaches the agent the correct patterns, constraints, and gotchas so it generates correct Kanji code every time.
...
## 3. Project Structure (Typical Kanji App)
...
## 4. How to Create a New Resource (Step by Step)
...
Services use the `Database` interface from `@kanjijs/store`. This interface is **database-agnostic** — the same service code works with both PostgreSQL and MongoDB. The query builder API is identical for both adapters.

#### Database-Agnostic Service (recommended — works with both Postgres and MongoDB)
...
// INSERT
const [created] = await this.db.query.users.insert({ name: 'Alice', email: 'alice@example.com' });
...
#### PostgreSQL-Only Service (uses Drizzle directly)

When you need Drizzle-specific features (complex joins, aggregations, `RETURNING`), you can use Drizzle directly in a Postgres-only service:
...
export class UsersServicePostgres {
  // Type the db as Drizzle's PostgresJsDatabase for Drizzle-specific methods
...
## 6. Dynamic Modules (Config Pattern)
...
providers: [
  ...
  // equivalent to just listing the class in providers array
]
```
...
### Use Error Classes from `@kanjijs/common`
```typescript
import {
  KanjiError,
  BadRequestError,
...
## 15. OpenAPI + SDK Generation Flow
