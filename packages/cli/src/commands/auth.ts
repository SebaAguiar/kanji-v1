...
export async function runAuthSetup(options: { dryRun: boolean }): Promise<void> {
  ...
  const resources: string[] = dirEntries
    .filter(
      (entry) =>
        entry.isDirectory() && !['auth', 'db', '__tests__', 'common', 'webhooks'].includes(entry.name),
    )
...
export function registerAuthCommand(program: Command) {
  program
    .command('auth-setup')
    .alias('g-auth')
    .description('Configure interactive security policies for existing modules (Deprecated)')
    .option('--dry-run', 'Preview changes without writing to disk', false)
    .action(async (options: { dryRun: boolean }) => {
      console.warn(
        pc.yellow(
          '\nWarning: "kanji auth-setup" (and "g-auth") is deprecated and will be removed in future versions.\n' +
            'Please use "kanji g auth" instead.\n',
        ),
      );
      await runAuthSetup(options);
    });
}
